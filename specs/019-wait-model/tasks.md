...
- [x] T001 Run `pytest test_flow.py` and confirm exactly 52 tests pass — Slice A gate
...
- [x] T002 [US1] Confirm `test_hld005_verify_invariant` is absent from `test_flow.py` (RED gate: grep returns nothing)
- [x] T003 [US1] Write `test_hld005_verify_invariant` in `test_flow.py`: docstring cites `"HLD-005 VERIFY"` and full text `"escalate and split both park the task as blocked and free the runner; a task is runnable only when it has no unmet dependencies"`; assert escalating a task transitions it to `blocked`; assert splitting a task transitions parent to `blocked`; assert `flow.next_task` is callable (returns None or a task) after both operations
- [x] T004 [US1] Run `pytest test_flow.py::test_hld005_verify_invariant` — confirm GREEN; then run `pytest test_flow.py` — confirm all 52 prior tests still pass

---

## Phase N: Polish

- [x] T005 Run `pytest test_flow.py` — confirm 53 total tests pass (SC-006 gate)
- [x] T006 Update `specs/019-wait-model/plan.md` Agent Context to reflect implementation complete
